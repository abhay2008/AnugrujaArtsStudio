...
export async function commitBinaryFile(
  ...
  if (!githubConfigured()) {
    throw new Error('GitHub credentials (GITHUB_TOKEN) are not configured');
  }
  ...
  const existing = await githubFetch(`${contentsUrl}?ref=${encodeURIComponent(branch())}&_t=${Date.now()}`);
  if (existing.ok) {
  ...
  if (put.status === 409) {
    const retryExisting = await githubFetch(`${contentsUrl}?ref=${encodeURIComponent(branch())}&_t=${Date.now()}`);
    if (retryExisting.ok) {
  ...
  if (!put.ok) {
    ...
    throw new Error(`GitHub commit failed for ${repo()} (${put.status}): ${err.slice(0, 300)}`);
  }
  ...
  return {
    ...
    htmlUrl: body.content?.html_url || `https://github.com/${owner()}/${repo()}`,
...
export async function getRemoteTextFile(filePath: string): Promise<string | null> {
  ...
  const contentsUrl = `/repos/${owner()}/${repo()}/contents/${encodedPath}?ref=${encodeURIComponent(branch())}&_t=${Date.now()}`;
  try {
